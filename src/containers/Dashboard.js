  ...
  async componentDidMount() {
    try {
      const userResponse = await axios.get(
        "https://spotify-api-artists-only.herokuapp.com/api/v1/user"
      );
      this.setState({ user: userResponse.data });
      const artistsResponse = await axios.get(
        "https://spotify-api-artists-only.herokuapp.com/api/v1/artists",
        {
          params: {
            ...
            <Grid.Column className="container-fsize">
              <Segment className="container-fsize container-color--black">
                <Header as="h1" size="huge" style={{ color: "white" }}>
                  <Image
                    circular
                    src={require("../common/images/spotify.png")}
                  />
                  Hello, {user.username}
                </Header>
